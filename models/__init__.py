...
def get_model(args, device, len_train_loader, transform):
    ...
    if args.model.name == 'simsiam':
        ...
        if args.model.proj_layers is not None:
            backbone.projector.set_layers(args.model.proj_layers)
    elif args.model.name == 'sidae':
        backbone = SidAE(get_backbone(args.model.backbone, args.dataset.name, args.cl_default)).to(device)
        if args.model.proj_layers is not None:
            backbone.projector.set_layers(args.model.proj_layers)
        if args.model.decoder is not None:
            backbone.set_decoder(args.model.decoder)

    names = {}
    all_models = get_all_models()
    all_models = list(filter(lambda x: x != '', all_models))  # remove empty
